...
class IsOwnerOrReadOnlyForPublicTest(TestCase):
    def setUp(self):
        ...
        self.owner = User.objects.create_user(
            email="owner@example.com", password="12345"
        )
    ...
    def test_safe_methods_allowed_for_public(self):
        for method in SAFE_METHODS:
            ...
            self.assertTrue(
                self.permission.has_object_permission(request, None, self.public_habit)
            )
    ...
    def test_safe_methods_allowed_for_owner_private(self):
        for method in SAFE_METHODS:
            ...
            self.assertTrue(
                self.permission.has_object_permission(request, None, self.private_habit)
            )
