...
class ValidatorsTest(TestCase):
    ...
    def test_validate_periodicity_valid(self):
        for valid_val in range(1, 8):
            try:
                ...
                self.fail(f"validate_periodicity() raised ValidationError unexpectedly for {valid_val}")
    ...
    def test_validate_periodicity_invalid(self):
        ...
        for val in invalid_values:
            ...
            self.assertIn(_("Периодичность должна быть от 1 до 7 дней включительно."), cm.exception.messages)
    ...
    def test_validate_time_to_complete_valid(self):
        ...
        try:
            ...
            self.fail("validate_time_to_complete() raised ValidationError unexpectedly for valid value")

    def test_validate_time_to_complete_zero_or_negative(self):
        with self.assertRaises(ValidationError) as cm:
            validate_time_to_complete(timedelta(seconds=0))
        self.assertIn(_("Время выполнения должно быть положительным числом."), cm.exception.messages)
    ...
    def test_validate_time_to_complete_above_max(self):
        ...
        self.assertIn(_("Время выполнения не должно превышать 120 секунд (2 минуты)."), cm.exception.messages)
    ...
    def test_validate_associated_habits_reward_and_associated_conflict(self):
        ...
        self.assertIn(_("Нельзя указывать одновременно вознаграждение и связанные привычки."), cm.exception.messages)
    ...
    def test_validate_associated_habits_non_pleasant_related(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            validate_associated_habits(self.habit)
        self.assertIn(_("Связанные привычки должны быть приятными."), cm.exception.messages)
    ...
    def test_validate_associated_habits_pleasant_habit_no_reward_or_associated(self):
        ...
        try:
            ...
            self.fail("Вызов full_clean() выбросил ошибку для приятной привычки без связанных привычек и награды")
